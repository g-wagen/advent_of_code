...
def worker_function(pos):
    return walk_everywhere(terrain=the_map, y=pos[0], x=pos[1])
...
if __name__ == "__main__":
    ...
    for i in range(1, max_steps):
        # data_list = positions_collection[-1]
        # results = []
        with multiprocessing.Pool(processes=max_processes) as pool:
            result = pool.map(worker_function, positions_collection[-1])
        ...
        #     )
        positions_collection.append(result2)
